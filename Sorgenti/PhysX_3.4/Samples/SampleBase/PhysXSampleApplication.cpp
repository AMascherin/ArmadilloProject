	...
	mDefaultSamplePath					(NULL)
{
	...
	mScreenQuadTopColor		= RendererColor(0x90, 0x5B, 0x44);
	mScreenQuadBottomColor	= RendererColor(0x90, 0x5B, 0x44);
...
void PhysXSampleApplication::onRender()
{
		...
		}
	{
		...
		if (mMenuExpand)
		{
			...
			const RendererColor highlightTextColor(158, 80, 36, 255);//highlightTextColor(255, 255, 0, 255);
			
			switch(mMenuType)
			{
                // TODO: modifiche per centrare il menù
				case MenuType::TESTS:
				{
                    const RendererColor color(0, 0, 0);
                    ScreenQuad sq;
					...
					renderer->print(x, y += y_value, strbuf, scale, shadowOffset, textColor);
					...
					for (Test::TestGroup* child = parent->getFirstChild(); child != NULL; child = parent->getNextChild(*child))
					{
                        ...
                        x_value = width/2 - (int)floorl(strlen(strbuf)*scale * 8.5);

						renderer->print(x_value, y += yInc, strbuf, scale, shadowOffset, (mSelected == child ? highlightTextColor : textColor));
					}
				}
				break;
			...
			{                // TODO: modifiche per centrare il messaggio "press here for help" 

				y += yInc;

                PxU32 test1, test2;
                renderer->getWindowSize(test1, test2);
                ...
                if (msg) {
                    x_value = test1 / 2 - (int)floorl(strlen(msg)*scale*9);
                    renderer->print(x_value, y += yInc, msg, scale, shadowOffset, textColor);
                }
				y+=yInc;
			}
			...
			if (mTextAlphaHelp != 0.0f)
			{
                ...
                if (m_platform->getSampleUserInput()->keyboardSupported() && m_platform->getSampleUserInput()->gamepadSupported()) {
                    renderer->print(x, y += yInc, "Use arrow keys or the left D-Pad to move", scale, shadowOffset, textColor);
				...
				//msg = inputInfoMsg("Press "," to run the game", MENU_SELECT, -1);
			/*	if(msg)
					renderer->print(x, y += (yInc+10), msg, scale, shadowOffset, textColor);
				msg = inputInfoMsg("Press "," to exit the menu", MENU_ESCAPE, -1);*/
				//if(msg)
				renderer->print(x, y += yInc, "Press ESC to exit the menu", scale, shadowOffset, textColor);
	...
	if(val)
	{
       // shdfnd::printFormatted("Value : %d\n", mMenuExpand);
		...
		case MENU_ESCAPE:		if(mMenuExpand)		menuKey = MenuKey::ESCAPE;		break;
            // TODO: disattivate le frecciette
		case MENU_QUICK_UP:		menuKey = MenuKey::NAVI_UP;		break;  //mMenuExpand = true;
		case MENU_QUICK_DOWN:	menuKey = MenuKey::NAVI_DOWN;	break;  //mMenuExpand = true;
		case MENU_QUICK_LEFT:	menuKey = MenuKey::NAVI_LEFT;	break;  //mMenuExpand = true;
		case MENU_QUICK_RIGHT:	menuKey = MenuKey::NAVI_RIGHT;	break;  //mMenuExpand = true;
		}
		if (mMenuExpand)
		{
